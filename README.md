...
1. 点击“新建项目”，填写项目名称与说明。
2. 在“实验方案”中新建方案名称和版本（如 V1、V2），并按行录入子实验；每个版本都会保存为项目根目录中的独立文件夹。
...
“导出项目”会下载一份完整的 Markdown 文件。其中会先以目录树列出项目的 Markdown 文件，再按原目录路径逐一汇总所有内容，因此版本、子实验、日志、对话和项目记忆的层级关系都能保留，适合检索或作为后续 AI 对话的上下文。

## 方案目录与附加文件

新建版本 `V1` 后，目录会直接位于项目根目录：`项目/V1/`。每个子实验是 `V1` 下的独立文件夹；关联到该子实验的日志会保存到它的 `实验日志/` 目录。方案页也可以直接在版本或子实验下新增 Markdown 文件和子文件夹。
