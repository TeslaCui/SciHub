...
6. checklist（已完成勾选）：**当且仅当该步骤没有等待时间（duration_hint 为空）**，且原文里有「需要逐项核对 / 重复多次 / 分多批」的可勾选事项时，把它整理成勾选条目。例如：
   - 原文「第一次抽滤（ ），第二次抽滤（ ）」→ checklist 为 ["第一次抽滤", "第二次抽滤"]
   - 原文「分别称取 A、B、C」且需要逐个确认 → ["称取 A", "称取 B", "称取 C"]
   - 条目用操作者能直接核对的动作命名，简洁（20 字以内），不要带编号/括号/单位。
   - **没有可勾选事项、或该步骤已有等待时间**，checklist 就给空数组 []。
7. title 字段：整个方案的名称（取文档标题，没有就用「未命名实验方案」）。
8. 只输出 JSON，不要任何解释文字或 Markdown 代码块。

输出 JSON 结构：
{"title":"方案名","steps":[{"title":"步骤标题","instruction":"操作要点","notice":"","duration_hint":"","checklist":["第一次抽滤"],"fields":[{"label":"字段名","unit":"g","type":"number"},{"label":"是否抽滤完成","unit":"","type":"check"}]}]}`;
