...
// ── 请求 / 响应 ─────────────────────────────────────────────
// POST { "text": "方案全文" }
// 200  { "title": "...", "steps": [ { "title", "instruction", "duration_hint",
//                                     "fields": [ { "label", "unit" } ] } ] }
...
3. instruction：把该步骤的操作要点整理成通顺的几条（用换行分隔），保留所有关键数值、条件、注意事项（如「不得完全密闭」「必要时超声 5 min」）。
4. duration_hint：若该步骤暗含等待时间（如「搅拌 24 h」「干燥过夜」「保温 1 h」），写成简短提示，如「约 24 小时」「需隔夜等待」；没有就填空字符串。
5. title 字段：整个方案的名称（取文档标题，没有就用「未命名实验方案」）。
6. 只输出 JSON，不要任何解释文字或 Markdown 代码块。

输出 JSON 结构：
{"title":"方案名","steps":[{"title":"步骤标题","instruction":"操作要点","duration_hint":"","fields":[{"label":"字段名","unit":"g"}]}]}`;
