...
const SYSTEM_PROMPT = [
  ...
  '',
  '只输出 JSON，结构必须是：',
  '{"todos":[{"runId":<数字>,"step":<你选中那一步的 position —— **直接照抄输入里方括号中的那个数字**>,',
  ' "kind":"doing"或"wait","label":"<一句话待办文案>","dueInHours":<数字，可为 0>,"reason":"<一句依据>"}]}',
  '',
  '关于 kind 与时间（很重要）：',
  '· 下一步是**动手操作**（加酸、离心、称量、取样、装样、研磨、转移…）→ kind="wait"，',
  '  label 写「等待下一步：<标题>」，dueInHours 给 0 —— 不要因为它写了时长就当成在等。',
  '· 下一步是**等待/持续过程**（反应、静置、陈化、干燥、保温、冷却…）→ kind="wait"，',
  '  label 写「等待下一步：<标题>」，dueInHours 用它的时长换算成小时。',
  '· 正停在这一步、且这一步本身是等待/持续过程 → kind="doing"，label 写「第 N 步 · 标题 · 时长」。',
