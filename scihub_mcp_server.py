...
class Gateway:
    ...
    def call(self, name: str, args: dict[str, Any]) -> dict[str, Any]:
        slug = str(args.get("projectSlug") or "").strip()
        project = self.project(slug) if slug else None
...
def handle_message(gateway: Gateway, message: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if method == "initialize":
        return {"jsonrpc": "2.0", "id": request_id, "result": {"protocolVersion": PROTOCOL_VERSION, "capabilities": {"tools": {}}, "serverInfo": {"name": "scihub-memory", "version": "1.0"}}}
    if method == "ping":
...
def main() -> None:
    ...
    gateway = Gateway(args.projects_root)
